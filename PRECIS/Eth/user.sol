...
contract userRecords {
  ...
  // user object
  user user_obj;
  //Internal function to conver genderType enum from string Внутренняя функция для преобразования перечисления genderType из строки
  ...
  //Internal function to convert genderType enum to string Внутренняя функция для преобразования перечисления genderType в строку
